...
std_devs[std_devs == 0] = 1 # when standard deviation at a snp is 0, set it to 1 to prevent NA values in LD matrix
    # this should not happen if MAF based filter is applied to SNPs. In our main simulations, we did not do a MAF based filter and kept all SNPs in each population
G /= std_devs
