...
for num_causal in "${num_causal_list[@]}"; do
    for job in "${jobs[@]}"; do
        for hsq in "${heritability[@]}"; do
            #out=/expanse/lustre/projects/ddp412/kakamatsu/MAGEPRO_SIMULATIONS_DIR/MAGEPRO_REVIEWS2_${num_causal}causal_correffects_cholesky_newld_retune_job${job} 
            out=/expanse/lustre/projects/ddp412/kakamatsu/MAGEPRO_SIMULATIONS_DIR/MAGEPRO_REVIEWS2_${num_causal}causal_correffects_cholesky_newld_job${job} 
	    rm -rf $out
            mkdir $out
